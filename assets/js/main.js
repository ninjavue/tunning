...
if (document.querySelectorAll('.createdAt')){
    let crAt = document.querySelectorAll('.createdAt')
    crAt.forEach(cr => {
        let tt = new Date(cr.getAttribute('data-time'))
        cr.textContent = tt.toLocaleString()
    })
}
if (document.querySelectorAll('.currency')){
    let currencyList = document.querySelectorAll('.currency')
    currencyList.forEach(currency => {
        currency.textContent = currency.getAttribute('data-value')==0 ? 'сум' : '$'
    })
}
